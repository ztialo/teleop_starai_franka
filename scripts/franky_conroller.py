...
class FrankyListener(Node):
    ...
    def eef_cb(self, msg: PoseStamped):
        ...
        elif self.controller.initial_eef_pose is not None and self.controller.leader_connected:
...
class FrankyController:
    ...
    def _move_gripper(self, width: float, speed = 0.02):
    ...
    def update_current_pose(self, msg: PoseStamped):
        ...
        dx = msg.pose.position.x - self.initial_eef_pose[0]
        dy = msg.pose.position.y - self.initial_eef_pose[1]
        dz = msg.pose.position.z - self.initial_eef_pose[2]
        ...
        q_rel = (Rotation.from_quat(q_current) * self.initial_q_inverse).as_quat()
    ...
    def update_gripper(self, width: float):
        self._move_gripper(width, self.gripper_speed)
